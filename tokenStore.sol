...
contract CCNCarnival {
    ...
    struct Stall {
        uint256 stallId;
        string stallName;
        address owner;           // Address of stall owner (student/staff)
    ...
    //payment with token
    function makeTokenPayment(uint256 _productId) public carnivalActive {
        Product memory product = products[_productId];
        require(product.productId != 0, "Product does not exist");
        require(isStallOperating(product.stallId), "Stall is not operating today");
        ...
        bool success = IERC20(tokenAddress).transferFrom(msg.sender, address(this), price);
